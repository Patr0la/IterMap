...
export class PreviewMarkerList extends React.Component<Props, State> {
	...
	renderMarkers() {
		...
		this.props.markers.forEach((marker, i) => {
			if ((marker.logicFunction == "day" && i > 0) || i == this.props.markers.length - 1) {
				i == this.props.markers.length - 1 && storedMarkers.push(marker);
				toRet.push(
					<View>
						<Text>Day {markerDay}</Text>
					</View>,
					<View>
						<Carousel
								...
								return (
									<View key={id} style={{ width: d.width * 0.8, height: d.height * 0.3, flexDirection: "row", alignItems: "center" }}>
										...
										<View style={{ backgroundColor: "white", width: "100%", minHeight: d.height * 0.15, flexDirection: "column", flex: 1, marginRight: "5%", elevation: 3, justifyContent: "space-around", padding: "4%" }}>
											...
											<Text style={{ color: "#242424", fontSize: 14, flex: 1, flexWrap: "wrap" }}>{description.substr(0,description.substr(0, 50).lastIndexOf(" "))} ...</Text>
	...
	render() {
		...
		return <ScrollView>{this.renderMarkers()}</ScrollView>;
	}
}
